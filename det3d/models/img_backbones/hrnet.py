...
from det3d.ops.mmseg_ops import Upsample, resize

# from ..builder import BACKBONES
from .resnet_mmcv import BasicBlock, Bottleneck
...
class HRNet(BaseModule):
    ...
    def load_pretrained_model(self):
        ...
        print("==> Loading pretrained weights for HRNet from {}".format(filename))
        ...
        if isinstance(checkpoint, OrderedDict):
            ...
            print("==> Loading checkpoint from OrderedDict")
            state_dict = checkpoint
        elif isinstance(checkpoint, dict) and "state_dict" in checkpoint:
            print("==> Loading checkpoint from dict")
            # checkpoint: meta, state_dict
        ...
        for key, val in self.state_dict().items():
            # NOTE: select the backbone parameters from the whole model
            global_key = 'backbone.' + key
            if key in state_dict:
                matched_val = state_dict[key]
            elif global_key in state_dict:
                matched_val = state_dict[global_key]
            else:
                print("==> No matched parameters for HRNet.{}".format_map(key))
            
            if val.shape == matched_val.shape:
                update_model_state[key] = matched_val
            else:
                print("==> Shape mismatch for matched HRNet.{}".format_map(key))

        
        self.load_state_dict(update_model_state, strict=False)
    ...
    def _freeze_stages(self):
        ...
        for i in range(1, self.frozen_stages + 1):
            ...
            for param in t.parameters():
                param.requires_grad = False
            print("==> stage {} frozen for HRNet".format(i))
