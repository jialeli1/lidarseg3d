...
class PolarNet_BEV_Unet(nn.Module):
    ...
    def __init__(self,n_class,n_height,dilation = 1,group_conv=False,input_batch_norm = False,dropout = 0.,circular_padding = False, dropblock = True, use_vis_fea=False):
        super(PolarNet_BEV_Unet, self).__init__()
    ...
    def forward(self, batch_dict):
        ...
        # [b, 32c, 480, 360] -> [b, 608=32*19, 480, 360]
        # print('==> 1. x.shape: ', x.shape)
        # ==> 1. x.shape:  torch.Size([4, 32, 480, 360])

        x = self.network(x)
        # print('==> 2. x.shape: ', x.shape)
        # ==> 2. x.shape:  torch.Size([4, 544=32*17, 480, 360]
        ...
        # ==> 3. x.shape:  torch.Size([4, 17, 480, 360, 32])
        # return x

        # batch_dict format
        batch_dict["voxel_features"] = x.permute(0,4,1,2,3)
...
class UNet(nn.Module):
    def __init__(self, n_class,n_height,dilation,group_conv,input_batch_norm, dropout,circular_padding,dropblock):
        ...
        # print(self.outc)
        # outconv(
        #     (conv): Conv2d(64, 608, kernel_size=(1, 1), stride=(1, 1))
        # )
