...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None, sync_bn_type="apex", distillation_flag=False):
    if logger is None:
    ...
    if distributed:
        if sync_bn_type == "apex":
            # this line can accelerate training, but cause some problems.
            # see https://github.com/NVIDIA/apex/issues/953
            model = apex.parallel.convert_syncbn_model(model)
            logger.info("Do run apex.parallel.convert_syncbn_model(model)")
        elif sync_bn_type == "torch":
            # see https://discuss.pytorch.org/t/correct-way-to-use-sync-batch-norm-for-using-apex-and-ddp/117780
