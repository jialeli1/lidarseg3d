...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True):
    ...
    for sample in tqdm(nusc.sample):
        ...
        # print("==> 0, ref_sd_token: ", ref_sd_token)
        # dce7af613063406eb380da056d58ce9d
        ...
        while len(sweeps) < nsweeps - 1:
            if curr_sd_rec["prev"] == "":
                if len(sweeps) == 0:
                    ...
                    if not test:
                        # 这一帧肯定是有的
                        # curr_ref_seganno_path = nusc.get('lidarseg', ref_sd_token)['filename']
                        curr_ref_seganno_path = None
                        # 更新这个key
                        sweep["seganno_path"] = curr_ref_seganno_path
                    sweeps.append(sweep)
                else:
        ...
        if not test:
            ...
            # 增加idarseg部分
            # 只记录路径, 不做解析, 在dataloader里面读取+解析
            # 添加到info里面
            ref_seganno_path = nusc.get('lidarseg', ref_sd_token)['filename']
            info["seganno_path"] = ref_seganno_path
            # print("==> ref_lidar_path: {}, ref_seganno_path: {}, ref_cam_paths: {}".format(ref_lidar_path, ref_seganno_path, ref_cam_paths))
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", nsweeps=10, filter_zero=True):
    ...
    if test:
        ...
        with open(
            root_path / "infos_test_{:02d}sweeps_segdet_withvelo.pkl".format(nsweeps), "wb"
        ) as f:
        ...
        with open(
            # root_path / "infos_train_{:02d}sweeps_segdet_withvelo_filter_{}_{:03d}rate.pkl".format(nsweeps, filter_zero, int(rate*100)), "wb"
            root_path / "infos_train_{:02d}sweeps_segdet_withvelo_filter_{}.pkl".format(nsweeps, filter_zero), "wb"
        ) as f:
            pickle.dump(train_nusc_infos, f)
        with open(
            root_path / "infos_val_{:02d}sweeps_segdet_withvelo_filter_{}.pkl".format(nsweeps, filter_zero), "wb"
        ) as f:
