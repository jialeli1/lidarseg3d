...
from det3d.ops.voxel.voxel_layer import dynamic_point_to_voxel_backward, dynamic_point_to_voxel_forward
# from det3d.ops.voxel import voxel_layer
...
class _dynamic_scatter(Function):
    ...
    def forward(ctx, points, coors, voxel_size, coors_range):
        ...
        Returns:
            ...
            max_points != -1.
        """
        results = dynamic_point_to_voxel_forward(points, coors, voxel_size, coors_range)
...
class DynamicScatterWithDistance(nn.Module):

    def __init__(self, voxel_size, point_cloud_range, pool_method='weighted_avg'):
        super(DynamicScatterWithDistance, self).__init__()
        """Scatters points into voxels, used in the voxel encoder with
           dynamic voxelization
        ...
        **Note**: The CPU and GPU implementation get the same output, but
        ...
        Args:
            pool_method: 
                'max': max pooling
                'avg': avg pooling
                'weighted_avg': distance-based weights guided avg pooling
    ...
    def forward_single(self, points, coors):
        """
        这里的distance实际上是1/distance，即weight
        """
        distances_voxels, voxel_coors, num_points = dynamic_scatter(
            points.contiguous(), coors.contiguous(), self.voxel_size,
            self.point_cloud_range)
        # 拆分出distance和feature
        distances = distances_voxels[..., 0].contiguous()
        voxels = distances_voxels[..., 1:].contiguous()
        ...
        if self.pool_method == 'max':
            ...
            voxels = (
                voxels.sum(dim=1, keepdim=False).div(num_points.view(-1, 1)))
        elif self.pool_method == 'weighted_avg':
            # TODO: 检查是否受到num_points的影响？
            # sum: (N, M) -> (N, 1)
            # div: (N, M) / (N, 1) -> (N, M) TODO: check?

            weights = distances / (distances.sum(dim=1, keepdim=True) + 1e-8)
            # (N, M, C) * (N, M, 1) -> (N, M, C) -> (N, C)
            voxels = torch.sum( voxels * weights.unsqueeze(dim=-1), dim=1, keepdim=False )
    ...
    def __repr__(self):
        ...
        tmpstr += 'voxel_size=' + str(self.voxel_size)
        tmpstr += ', point_cloud_range=' + str(self.point_cloud_range)
        tmpstr += ', pool_method=' + str(self.pool_method)
        tmpstr += ')'
        return tmpstr