...
__global__ void ball_query_with_count_kernel_stack(int B, int M, float radius, int nsample, \
    const float *new_xyz, const float *new_label, const int *new_xyz_batch_cnt, \
    const float *xyz, const float *label, const int *xyz_batch_cnt, \
    int *count) {
    // :param xyz: (N1 + N2 ..., 3) xyz coordinates of the features
    // :param label: (N1 + N2 ..., ) sem labels
    // :param xyz_batch_cnt: (batch_size), [N1, N2, ...]

    // :param new_xyz: (M1 + M2 ..., 3) centers of the ball query
    // :param new_label: (M1 + M2 ..., 1) sem label of the centers
    // :param new_xyz_batch_cnt: (batch_size), [M1, M2, ...]
    ...
    // 将统计到的local_count赋值和return 
    count[0] = local_count;

    // 如果没有满足半径的点, 直接对count置零
    // if (cnt == 0) idx[0] = -1;
    // if (cnt == 0) count = 0;
...
void ball_query_with_count_kernel_launcher_stack(int B, int M, float radius, int nsample,
    const float *new_xyz, const float *new_label, const int *new_xyz_batch_cnt, 
    const float *xyz, const float *label, const int *xyz_batch_cnt, 
    int *count){
    // :param xyz: (N1 + N2 ..., 3) xyz coordinates of the features
    ...
    ball_query_with_count_kernel_stack<<<blocks, threads>>>(B, M, radius, nsample, new_xyz, new_label, new_xyz_batch_cnt, xyz, label, xyz_batch_cnt, count);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
        fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
