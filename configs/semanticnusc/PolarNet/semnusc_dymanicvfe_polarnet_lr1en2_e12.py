...
model = dict(
    ...
    pretrained=None,
    reader=dict(
        type="PolarNetDynamicVoxelFeatureExtractor", 
        grid_size=cylindrical_grid_size,
        point_cloud_range=cylindrical_range, 
        average_points=False,                   # from polarnet
        num_input_features=5, 
        num_output_features=512,                # from polarnet
        fea_compre=cylindrical_grid_size[-1],   # from polarnet
    ),
    backbone=dict(
        type="PolarNet_BEV_Unet", 
        n_class=num_class,
        n_height=cylindrical_grid_size[-1],
        input_batch_norm=True,
        dropout=0.5,
        circular_padding=True,
        use_vis_fea=False,
        # num_input_features=512,               # fixed as n_height in PolarNet_BEV_Unet
    ),
    point_head=dict(
        type="PointSegPolarNetHead",            
...
train_preprocessor = dict(
    ...
    global_scale_noise=[0.95, 1.05], #ori: [0.95, 1.05]
